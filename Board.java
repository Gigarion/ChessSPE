...
public class Board
{
    ...
    public void move(Piece toMove, Tile target)
    {
        ...
        if (target.getPiece() != null)
        {
            ...
            // cancels appropriate castling rights if the rook is captured
            if (target.getPiece().getType() == 'R' && Character.getNumericValue(target.getID().charAt(1)) == 7) {
                game.getCastlingRights()[0] = false;
            }
            else if (target.getPiece().getType() == 'R' && Character.getNumericValue(target.getID().charAt(1)) == 0) {
                game.getCastlingRights()[1] = false;
            }
            else if (target.getPiece().getType() == 'r' && Character.getNumericValue(target.getID().charAt(1)) == 7) {
                game.getCastlingRights()[2] = false;
            }
            else if (target.getPiece().getType() == 'r' && Character.getNumericValue(target.getID().charAt(1)) == 0) {
                game.getCastlingRights()[3] = false;
            }
        ...
        else if (toMove.getType() == 'p' && (targetrank - piecerank) == -2) {
            game.setEP(getTile(targetrank + 1, piecefile));
            game.setHalfMoves(0);
        }
